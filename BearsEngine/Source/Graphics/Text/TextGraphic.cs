...
public class TextGraphic : RectGraphicBase, IDisposable
{
        ...
        : base(shader, x, y, w, h)
    {
        ...
        Ensure.ArgumentNotNegative(h, nameof(h));
        Ensure.ArgumentNotNegative(scaleX, nameof(scaleX));
        Ensure.ArgumentNotNegative(scaleY, nameof(scaleY));

        _font = font;
        Colour = colour;
        _text = text;
        _numCharsToWrite = text.Length;
        ScaleX = scaleX;
        ScaleY = scaleY;
            ...
            foreach (var com in line)
            {
                ...
                else if (com is LC_Word w) //if it's a newline nothing will happen
                {
                    for (int i = 0; i < w.Text.Length; ++i)
                    {
                        if (w.Font != lastFont)
                        {
                            _vertGroups.Add(new SimpleGraphic(new DefaultShader(), OpenGLHelper.LoadTexture(lastFont.CharSpriteSheet, lastFont.LongName), vertices.ToArray()));
    ...
    private void SetVerticesSimple()
    {
        ...
        if (vertices.Count > 0)
            _vertGroups.Add(new SimpleGraphic(new DefaultShader(), OpenGLHelper.LoadTexture(Font.CharSpriteSheet, Font.LongName), vertices.ToArray()));
