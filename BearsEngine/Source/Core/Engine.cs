...
{
    private static IGameEngine? s_instance;
    private static bool s_runCalled;
    ...
    /// <exception cref="InvalidOperationException">Throws if Engine has not been created.</exception>
    public static void Run(ApplicationSettings appSettings, Func<IScene> getFirstScene)
    {
        if (s_runCalled)
        {
        ...
        Instance.Run(AppWindow.Instance);
